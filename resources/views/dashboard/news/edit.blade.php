...
<div class="bg-white rounded-lg shadow-md p-6">
    ...
    <form action="{{ route('news.update', $news) }}" method="POST" enctype="multipart/form-data" class="max-w-4xl">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500">PNG, JPG, GIF (MAX. 2MB). Kosongkan jika tidak ingin mengubah.</p>
                @error('thumbnail')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
        ...
        <div class="mb-6">
            ...
            <textarea id="deskripsi" 
                      ...
                      required>{{ old('deskripsi', $news->deskripsi) }}</textarea>
            @error('deskripsi')
                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
